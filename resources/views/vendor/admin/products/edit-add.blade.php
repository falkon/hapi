                                    ...
                                    @else
                                        <div class="form-group @if($row->type == 'hidden') hidden @endif @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            ...
                                            @if($row->field == 'category_id')
                                                @include('admin::formfields.select_dropdown', ['customParameter' => 1])
                                            @else
                ...
                <div class="col-md-12">
                    <div class="panel panel-bordered">
                        ...
                        @foreach($notformDesignerFields as $row)
                            ...
                            @php
                                $model = app('App\ProductParameter');
                                $query = $model::where('product_id', '=', $dataTypeContent->id)->get();
                                $relationDataType = Admin::model('DataType')->where('name', '=', $model->getTable())->first();
                            @endphp

                            <label for="name">Параметры товара</label>
                            @section('datatable_header1')
                            ...
                            @include('admin::list.datatable-my', [
...
@section('javascript2')
    ...
    <script>
        $(document).ready(function(){
            $('body').find('#edit-input_image')[0].click();
        });
    </script>
    @parent
@endsection
